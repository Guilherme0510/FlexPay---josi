...
export const Add: React.FC = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      setFormData({ valor: 0, data: new Date(), observacoes: "", tipo: "positivo", categoria: '' });
    } catch (error) {
  ...
  id="data"
  name="data"
  value={formData.data.toISOString().split('T')[0]} // Aqui convertemos a data para o formato 'YYYY-MM-DD'
  onChange={(e) => setFormData({ ...formData, data: new Date(e.target.value) })} // Convertendo de volta para objeto Date
  className="form-control"
/>
          </div>
          <div className="form-group">
